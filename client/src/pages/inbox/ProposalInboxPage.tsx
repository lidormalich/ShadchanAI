...
import { AlertTriangle, Check, Clock, ExternalLink, Inbox, Lock, Minus, RotateCcw, TrendingDown, TrendingUp, X } from 'lucide-react';
...
const TABS: Array<{ id: TabId; label: string; icon: React.ReactNode }> = [
  ...
  { id: 'closed',       label: 'נסגרו',   icon: <Lock className="h-4 w-4" /> },
];
...
export function ProposalInboxPage() {
  ...
  const queries = {
    ...
    closed: useTabQuery('closed', direction, minScore),
  };
    ...
    <div className="space-y-4">
      ...
      {active.isLoading ? (
        ...
        <EmptyState
          ...
          title={
            tab === 'inbox' ? 'התיבה ריקה'
              : tab === 'review_later' ? 'אין הצעות בשהייה'
                : tab === 'closed' ? 'אין הצעות שנסגרו'
                  : 'אין הצעות שנדחו'
          }
  ...
  return (
    <li className="py-2.5 px-3">
        ...
        <div className="text-xs text-ink-muted flex items-center gap-2 flex-wrap mt-0.5">
          ...
          {tab === 'closed' && (
            <Badge tone="neutral" icon={<Lock className="h-3 w-3" />}>
              {row.closeStatus === 'expired' ? 'פג תוקף' : 'נסגר'}
            </Badge>
          )}
        </div>
        {tab !== 'inbox' && (tab === 'closed' ? row.closeReason : row.reviewReason) && (
          <div className="text-xs text-ink-faint mt-1 truncate" title={tab === 'closed' ? row.closeReason : row.reviewReason}>
            <span className="text-ink-muted">סיבה:</span> {tab === 'closed' ? row.closeReason : row.reviewReason}
          </div>
        )}
      </div>
      ...
      <div className="shrink-0 flex items-center flex-wrap gap-1.5">
        {/* A closed pair was already decided — its only action is to open the
            closed suggestion itself; the decision buttons don't apply. */}
        {tab === 'closed' ? (
          row.matchSuggestionId && (
            <Link
              to={`/matches/${row.matchSuggestionId}`}
              className="inline-flex items-center gap-1.5 rounded-md h-8 px-3 text-xs font-medium bg-white border border-border text-ink hover:bg-bg-hover"
            >
              <ExternalLink className="h-3.5 w-3.5" /> כניסה להצעה
            </Link>
          )
        ) : (
          <>
